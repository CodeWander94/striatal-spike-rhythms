...
%Naive acceleration calculation. Acceleration at a given point is 
...
if cfg_master.plotOutput
    ...
    for iM = 1:length(mn)
        ...
        subplot(222)
        %[ax h1 h2] = plotyy(cfg_ttr.bins,lambda_mttr,cfg_ttr.bins,lambda_spd); %Commented because of mismatch in lengths X and Y
        temp_X = 0.5 * (cfg_ttr.bins(1:end-1) + cfg_ttr.bins(2:end));
        %[ax h1 h2] = plotyy(cfg_ttr.bins,lambda_mttr,cfg_ttr.bins,lambda_spd);
        [ax h1 h2] = plotyy(temp_X,lambda_mttr,temp_X,lambda_spd);
        hold on;
        set(h1,'LineWidth',2);
        set(gca,'XTick',-4.95:5,'LineWidth',1,'FontSize',18); box off;
