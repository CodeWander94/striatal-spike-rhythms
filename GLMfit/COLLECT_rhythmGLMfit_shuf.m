...
for iS = 1:nSessions
    ...
    for iM = 1:nModels
        ...
        this_diff = sd.m.baseline.err(keep, :) - sd.m.(cfg.models{iM}).shufErr(keep, :);
        ALL_avgErrShuf(iM, start_idx:end_idx) = this_diff;
        
        % ttr err
        ALL_ttrErr(iM, start_idx:end_idx, :) = sd.m.(cfg.models{iM}).ttr_err(keep, :);
        ALL_ttrErrShuf(iM, start_idx:end_idx, :) = sd.m.(cfg.models{iM}).ttr_errShuf(keep, :);
        
        % space err 
        ALL_spaceErr(iM, start_idx:end_idx, :) = sd.m.(cfg.models{iM}).linpos_err(keep, :);
        ALL_spaceErrShuf(iM, start_idx:end_idx, :) = sd.m.(cfg.models{iM}).linpos_errShuf(keep, :);
...
for iM = 1:nModels
    ...
    % shuffle vs real
    subplot(221);
    plot(ALL_avgErr(iM, cell_keep), ALL_avgErrShuf(iM, cell_keep), '.k');
    
    this_err = ALL_avgErr(iM, cell_keep);
    errM = nanmean(this_err); errSD = nanstd(this_err);
    
    this_err = ALL_avgErrShuf(iM, cell_keep);
    errShufM = nanmean(this_err); errShufSD = nanstd(this_err);
    title(sprintf('Model %s (%s) dErr %.2e +/- %.2e, dErrShuf %.2e +/- %.2e\n', ...
        cfg.models{iM}, cellTypes{iC}, errM, errSD, errShufM, errShufSD));
    ...
    errZ = errM ./ errSD;
    p = signrank(this_err);
    fprintf('Model %s SHUF (%s) dErr %.2e +/- %.2e (z = %.2f), p = %.3e (nCells = %d)\n', cfg.models{iM}, cellTypes{iC}, errM, errSD, errZ, p, cellCount);
    ...
    this_ttrShuf = sq(ALL_ttrErrShuf(iM, cell_keep, :));
    ...
    %axes(ax(1)); 
    hold on; 
    plot(xvec, nanmean(this_ttrShuf), 'Color', [0.7 0.7 0.7]);
    plot(xvec, nanmean(this_ttrShuf) + 2*nanstd(this_ttrShuf), ':', 'Color', [0.7 0.7 0.7]);
    plot(xvec, nanmean(this_ttrShuf) - 2*nanstd(this_ttrShuf), ':', 'Color', [0.7 0.7 0.7]);
    
    if iM == nModels % 'all'
        set(gca,'XTick',-5:5,'LineWidth',1,'FontSize',18,'YLim',[-0.5e-6 5e-6],'YTick',0:1e-6:5e-6, 'TickDir', 'out'); box off;
    else
        set(gca,'XTick',-5:5,'LineWidth',1,'FontSize',18,'YLim',[-0.5e-6 2e-6],'YTick',0:1e-6:2e-6, 'TickDir', 'out'); box off;
    end
    ...
    this_ttr = normalizeM(this_ttr); this_ttrShuf = normalizeM(this_ttrShuf);
