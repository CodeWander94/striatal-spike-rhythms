...
for iM = 1:length(models)
    ...
    ax.TickDir = 'out';
    ax.YLim = [0 150];
    output_name = strcat('Pre task', {' '}, (models{iM}),' - type 1');
    ...
    ax.TickDir = 'out';
    ax.YLim = [0 100];
    output_name = strcat('Pre task', {' '}, (models{iM}),' - type 2');
    title(output_name, 'fontsize',30);
    xlabel('Model Improvement', 'FontSize', 15);
    WriteFig(output_name{1})
    figure;
    ...
    ax.TickDir = 'out';
    ax.YLim = [0 250];
    output_name = strcat('Pre task', {' '}, (models{iM}),' - type 1 and 2');
    title(output_name, 'fontsize',30);
    xlabel('Model Improvement', 'FontSize', 15);
    WriteFig(output_name{1})
%     figure;
...
%     text(0.00002,20,strcat('Percentage of improved models :',{' '}, num2str(round(pc,2))),'fontsize',15);
%     ax = gca;
%     ax.Box = 'off';
%     ax.FontSize = 15;
%     ax.TickDir = 'out';
%     ax.YLim = [0 25];
%     output_name = strcat('Pre task (Real - shuffle) ', {' '}, (models{iM}),' - type 1');
...
%     hold on;
%     pc = 100 * sum(mask)/length(mask);
%     text(0.00002,6,strcat('Percentage of improved models :',{' '}, num2str(round(pc,2))),'fontsize',15);
%     ax = gca;
%     ax.Box = 'off';
%     ax.FontSize = 15;
%     ax.TickDir = 'out';
%     ax.YLim = [0 8];
%     output_name = strcat('Pre task (Real - shuffle) ', {' '}, (models{iM}),' - type 2');
%     title(output_name, 'fontsize',25);
%     xlabel('Model Improvement', 'FontSize', 15);
%     hold off;
%     WriteFig(output_name{1})
...
%     ax.TickDir = 'out';
%     ax.YLim = [0 25];
%     output_name = strcat('Pre task (Real - shuffle) ', {' '}, (models{iM}),' - type 1 and 2');
%     title(output_name, 'fontsize',25);
%     xlabel('Model Improvement', 'FontSize', 15);
%     hold off;
%     WriteFig(output_name{1})
